  ...
  async function handleConnect() {
    ...
    try {
      const res = await fetch(`/api/admin/providers/${props.slug}/keys`, {
        ...
        body: JSON.stringify({ apiKey, label: props.name }),
      });
      if (!res.ok) {
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 mx-4 w-full max-w-md rounded-[14px] border border-[rgba(15,15,14,0.08)] bg-white p-6 shadow-xl">
            ...
            {error && (
              <div className="mb-3 rounded-[10px] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>
            )}
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={`Enter ${props.name} API key`}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleConnect} disabled={saving || !apiKey.trim()} className="flex-1 rounded-[10px] bg-[#0F0F0E] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#3A3A37] disabled:opacity-50">

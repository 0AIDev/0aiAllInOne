...
const PLANS = [
  {
    ...
    rpm: "60",
    keys: 5,
    features: ["100K tokens", "60 RPM", "5 API keys"],
  },
  {
    id: "STARTER",
    name: "STARTER",
    ...
    rpm: "300",
    keys: 20,
    features: ["500K tokens", "300 RPM", "20 API keys"],
  },
  {
    id: "PRO",
    name: "PRO",
    ...
    rpm: "1000",
    keys: 100,
    features: ["2M tokens", "1000 RPM", "100 API keys"],
  },
  {
    id: "BUSINESS",
    name: "ENTERPRISE",
    ...
    rpm: "Custom",
    keys: "Unlimited",
    features: ["Unlimited tokens", "Custom RPM", "Unlimited API keys"],
  },
];
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ ...form, planTier: selectedPlan }),
      });
        ...
        <div className="w-full max-w-3xl">
          <div className="rounded-[14px] border border-[rgba(15,15,14,0.08)] bg-white p-8">
                ...
                return (
                  <button
                    ...
                    className={`rounded-[14px] border p-5 text-left transition-all ${
                      isSelected
                        ? "border-[#0F0F0E] ring-1 ring-[#0F0F0E]"
                        : "border-[rgba(15,15,14,0.08)] hover:border-[rgba(15,15,14,0.2)]"
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm font-medium text-[#0F0F0E]">{plan.name}</p>
                        <p className="mt-1 text-2xl font-semibold tracking-tight text-[#0F0F0E]">
                          {plan.price}
                          {plan.period && <span className="text-sm font-normal text-[#7A7870]">{plan.period}</span>}
                        </p>
                      </div>
                      {isSelected && (
                        <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#0F0F0E]">
                    ...
                    <ul className="mt-4 space-y-1.5">
                      {plan.features.map((f) => (
                        <li key={f} className="text-xs text-[#7A7870]">
                          {f}
                        </li>
                      ))}
                    </ul>
            ...
            <button
              ...
              className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-[10px] bg-[#0F0F0E] px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-[#3A3A37] disabled:opacity-50"
            >
      ...
      <div className="w-full max-w-md">
        <div className="rounded-[14px] border border-[rgba(15,15,14,0.08)] bg-white p-8">
          ...
          <button
            ...
            className="mb-6 inline-flex items-center gap-1 text-sm text-[#7A7870] transition-colors hover:text-[#0F0F0E]"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="rounded-[10px] border border-[rgba(15,15,14,0.08)] bg-[#F9F9F6] px-4 py-3">
              <p className="text-xs text-[#7A7870]">
                Selected plan: <span className="font-medium text-[#0F0F0E]">{selectedPlan}</span>
              </p>
            </div>
            <button
              type="submit"
